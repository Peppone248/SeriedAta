...
### Phase 3 — Rescue path with defensive action features

Once defensive match data is fetched from FBref (via `soccerdata` misc
stat_type), the bridge can be refit with the actual defensive actions
that DefR measures — not just pass-volume proxies:

- Rescue bridge (FBref-standard + defensive: 17 features): CV R² = **0.83**
- Sanity check on Wyscout data confirms the proxy decouples from team
  quality: max correlation with quality indicators drops from 0.82
  (reduced) to **0.38** (rescue)
- However, walk-forward on 2020-2024 FBref data still shows zero F1
  improvement — confirms the conclusion that defensive style does not
  add information to W/D/L prediction beyond what existing strength
  features already provide

### Phase 4 — Team profiling and tactical clustering

Pivoting from prediction to description, this phase asks not whether
DefR predicts results but what defensive identities exist in Serie A
and how they evolve:

- Cross-season profiles for 2020–2024, one row per team-season
- K-means clustering (K = 4-6 chosen by silhouette) reveals **4
  tactical archetypes**: Dominant active defender, Compact mid-block,
  Balanced/transitional, Low block under pressure
- Stable identities confirmed empirically: Atalanta stays the same
  archetype every season under Gasperini
- Tactical shifts caught by the data: Bologna's Motta transformation
  (2022 → 2023), Juventus's shift in 2024 under Motta, Napoli's
  archetype change under Conte (2024)
...
# Step 3: Walk-forward validation (requires matches_classification pipeline)
...
### Run Phase 3 (rescue path — full bridge with defensive features)

```bash
cd SerieAwithPandas/defr

# Optional pre-check: confirm the rescue is worth pursuing on Wyscout data
python sanity_check_rescue.py

# Step 1: refit the bridge using defensive features (tackles, interceptions, fouls)
# from Wyscout events
python refit_bridge_rescue.py

# Step 2: fetch misc match data from FBref via soccerdata (needs browser)
python fetch_defensive_data.py

# Step 3: apply the rescue bridge (R² = 0.83)
python inject_defr_full.py

# Step 4: walk-forward comparing baseline vs reduced vs full
python walkforward_validate_full.py
```

### Run Phase 4 (team profiling and tactical clustering)

This phase pivots from prediction to description. It builds defensive
identities for every team-season in 2020–2024 and groups them into
tactical archetypes via K-means.

```bash
cd SerieAwithPandas/defr

# Optional enrichment: fetch season-level defense stats (tackles by third)
python fetch_defense_season_data.py

# Build team-season profiles and run clustering
python build_profiles_and_cluster.py

# Generate plots and HTML report
python make_profile_report.py
```

The profile builder gracefully degrades if optional inputs are missing:
- Without fetch_defensive_data.py output: uses only base FBref features
- Without fetch_defense_season_data.py output: skips tackles-by-third
- Without rescue-bridge proxy: falls back to the reduced-bridge proxy
